...
class FractalBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_columns, loc_drop_prob, dropout_prob):
        ...
        for i in range(depth):
            level = nn.ModuleList()   #level = nn.Sequential()
            for j in range(self.num_columns):
                if (i + 1) % (2 ** j) == 0:
                    in_ch_ij = in_channels if (i + 1 == 2 ** j) else out_channels
                    level.append(     #add_module(
                                    # f"subblock{j + 1}",
            ...
            blocks.append(level)
        self.blocks = nn.ModuleList(blocks)    #nn.Sequential(*blocks)
        self.use_checkpoint_per_subblock = False

    def forward(self, x):
        outs = [x for _ in range(self.num_columns)] #outs = [x] * self.num_columns
        for level in self.blocks:                   # for level_block in self.blocks:
            new_outs = outs.copy()
            active_cols = []
            active_outs = []
            for col_idx, blk in enumerate(level):
                    ...
                    if self.use_checkpoint_per_subblock:
                        out = cp.checkpoint(blk, inp, use_reentrant=False) #outs_i = [cp.checkpoint(blk, inp, use_reentrant=False) for blk, inp in zip(level_block, outs)]
                    else:
                        out = blk(inp)      #outs_i = [blk(inp) for blk, inp in zip(level_block, outs)]
                    
                    new_outs[col_idx] = out
                    if not isinstance(blk, nn.Identity):
                        active_cols.append(col_idx)
                        active_outs.append(out)
                    # ---- FRACTAL SKIP CONNECTION ----
            if len(active_outs) > 1:
                joined = torch.stack(active_outs, dim=0).mean(dim=0) #joined = torch.stack(outs_i, dim=0).mean(dim=0)
                for col_idx in active_cols:
                    new_outs[col_idx] = joined                       # outs[:len(level_block)] = [joined] * len(level_block)
