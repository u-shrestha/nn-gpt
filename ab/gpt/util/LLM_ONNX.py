...
class LLM:
    def __init__(self,
        ...
        # Check if the model_path is a direct local folder first (e.g. "onnx_output_folder")
        if os.path.isdir(model_path):
            real_model_path = model_path
            tokenizer_path = model_path
        else:
            # Fallback to the standard internal directory structure (out/llm/...)
            real_model_path = llm_dir(base_path, model_path)
            tokenizer_path = llm_tokenizer_dir(base_path, model_path)

        tokenizer_exists = exists(tokenizer_path)

        # Load Tokenizer
        # self.tokenizer = AutoTokenizer.from_pretrained(
        #     tokenizer_path if tokenizer_exists else model_path,
        #     trust_remote_code=True, token=access_token, gguf_file=gguf_file
        # )


        #  FIX: download the correct tokenizer from the internet
        print("Forcing tokenizer download from Hugging Face...")
        self.tokenizer = AutoTokenizer.from_pretrained(
            "Qwen/Qwen2.5-7B-Instruct",  # We point to the online repo directly
        ...
        if tokenizer_exists:
            print("Loading Tokenizer from local files:", tokenizer_path)
        else:
            print("Downloading Tokenizer...")
            # If it's a raw path, we assume tokenizer is already there or we don't save it to internal dir
            if not os.path.isdir(model_path): 
                self.tokenizer.save_pretrained(tokenizer_path, access_token=access_token)
                print("Tokenizer saved to: ", tokenizer_path)

        # --- Check for ONNX ---
        is_onnx = False
        if os.path.exists(os.path.join(real_model_path, "model.onnx")):
            is_onnx = True
        
        # --- Load Model ---
        if is_onnx:
            if not OPTIMUM_AVAILABLE:
                raise ImportError("Found an ONNX model but 'optimum' is not installed. Run: pip install optimum[onnxruntime-gpu]")
            ...
            self.model = ORTModelForCausalLM.from_pretrained(
                ...
                provider_options={
                "device_id": 0,
                # "gpu_mem_limit": ... (You can remove this if you killed the zombies)
                "arena_extend_strategy": "kSameAsRequested",
            }
            )
            if "CUDAExecutionProvider" not in self.model.model.get_providers():
                raise RuntimeError("CRITICAL: ONNX Runtime failed to load CUDA! Check your install.")
        else:
            # Standard PyTorch Loading
            print(f"Loading PyTorch Model from: '{real_model_path}'")
            
            # Helper to sanitize config if needed
