...
PIPELINE_NNEVAL_DIR = None  # Pipeline-specific: direct path to nneval directory (e.g., out/iterative_cycles_v3/cycle_1/nneval)
...
def main(nn_name_prefix=NN_NAME_PREFIX, nn_train_epochs=NN_TRAIN_EPOCHS, only_epoch=ONLY_EPOCH, save_to_db=SAVE_TO_DB,
         ...
         transform=TRANSFORM, epoch_limit_minutes=EPOCH_LIMIT_MINUTES, custom_synth_dir=CUSTOM_SYNTH_DIR, cycle=CYCLE, pipeline_nneval_dir=PIPELINE_NNEVAL_DIR):
    base_nngpt_path = nngpt_dir  # out/nngpt
    
    # Pipeline mode: if pipeline_nneval_dir is provided, use it directly
    if pipeline_nneval_dir:
        models_base_dir = Path(pipeline_nneval_dir)
        if not models_base_dir.exists():
            print(f"Pipeline nneval directory not found: {models_base_dir}", file=sys.stderr)
            return
        
        print(f"\n--- Pipeline Mode: Evaluating models in {models_base_dir} ---")
        current_cycle = cycle if cycle is not None else 1
        current_epoch = 0  # Pipeline doesn't use epoch concept
        cycle_start_time = time.time()
        current_alter_epoch_path = models_base_dir.parent  # Use parent as alter epoch path for compatibility
        
        # Process models directly (skip epoch loop)
        process_pipeline_models = True
    else:
        # Original mode: use epoch_dir structure
        process_pipeline_models = False
    ...
    # Process models - either pipeline mode or epoch loop mode
    if process_pipeline_models:
        # Pipeline mode: process models directly (already set up above)
        # models_base_dir, current_cycle, current_epoch, cycle_start_time, current_alter_epoch_path already set
        epochs_to_process = [0]  # Single iteration for pipeline mode
    elif nn_alter_epochs:
        # Original mode: process through epoch loop
        epochs_to_process = [only_epoch] if only_epoch is not None else range(nn_alter_epochs)
    else:
        epochs_to_process = []
    
    # Process each epoch/cycle
    for i in epochs_to_process:
        if not process_pipeline_models:
        ...
        # Shared model processing code (executes for both pipeline mode and epoch mode)
        if not models_base_dir.exists():
        ...
        for model_id in os.listdir(models_base_dir):
            ...
            # For pipeline mode, use relative path from models_base_dir; for epoch mode, use base_nngpt_path
            try:
                rel_path = model_dir_path.relative_to(base_nngpt_path)
            except ValueError:
                # If not relative to base_nngpt_path (pipeline mode), use relative to models_base_dir
                rel_path = model_dir_path.relative_to(models_base_dir)
            print(f"\n--- Evaluating Model: {rel_path} ---")
        ...
        # Generate cycle results JSON after all evaluations complete (for this epoch/cycle)
        ...
        # For pipeline mode, save to the cycle directory; otherwise save to base_nngpt_path
        if pipeline_nneval_dir:
            cycle_results_path = current_alter_epoch_path / "cycle_results.json"
        else:
        ...
        # If pipeline mode, we're done (only one iteration)
        if process_pipeline_models:
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pipeline_nneval_dir', type=str, default=PIPELINE_NNEVAL_DIR,
                        help="Pipeline mode: Direct path to nneval directory (e.g., out/iterative_cycles_v3/cycle_1/nneval). When set, bypasses epoch_dir structure.")
    ...
    main(args.nn_name_prefix, args.nn_train_epochs, args.only_epoch, args.save_to_db, args.nn_alter_epochs,
         ...
         args.custom_synth_dir, args.cycle, args.pipeline_nneval_dir)
